...
allowlist (`enforce_role_allowlist`), which only restricts the plain `user`
role. The aggregation query is Postgres-specific and cannot run against the
SQLite test DB, so the policy is asserted at the allowlist layer that runs
before any SQL.
"""
