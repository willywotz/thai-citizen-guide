...
@router.post(
    ...
    summary="Create a golden question for an agency (owner or admin)",
)
async def create_golden_question(
    agency_id: str, body: GoldenQuestionCreate, user: User = Depends(get_current_user)
...
@router.get(
    ...
    summary="List golden questions for an agency (owner or admin)",
)
async def list_golden_questions(
    agency_id: str, user: User = Depends(get_current_user)
...
@router.delete(
    ...
    summary="Delete a golden question (owner or admin)",
)
async def delete_golden_question(
    agency_id: str, gq_id: uuid.UUID, user: User = Depends(get_current_user)
) -> None:
    agency = await _get_agency_or_404(agency_id)
    await authorize_or_403(user, "agency:edit", agency)
...
@router.get(
    ...
    summary="Recent eval results for an agency's golden questions (owner or admin)",
)
