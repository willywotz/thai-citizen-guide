...
- [ ] **Step 5: Create the migration — the correct aerich way**

Follow `docs/aerich-migrations.md`. Do NOT hand-author or carry `MODELS_STATE` forward. Generate it against a DB that has all prior migrations applied so the snapshot is correct:
```bash
cd backend
# ensure DATABASE_URL points at a reachable Postgres (docker compose Postgres or a scratch DB)
.venv/bin/aerich upgrade                          # apply migrations 0..19 first
.venv/bin/aerich migrate --name llm_providers_routes
```
Then **review per the guideline**:
- `git status` shows exactly ONE new file `backend/migrations/models/20_*_llm_providers_routes.py` and **nothing modified/deleted** (if an existing migration changed, restore it and stop).
- The generated `upgrade()` must `CREATE TABLE` both `llm_providers` and `llm_routes` (with the `provider_id` FK `ON DELETE RESTRICT`); `downgrade()` drops both. These are plain tables — no `CREATE EXTENSION` or expression indexes to hand-add.
- `MODELS_STATE` was regenerated by aerich (differs from migration 19's).

Apply and confirm: `.venv/bin/aerich upgrade`.

If no DB is reachable at all, fall back to the guideline's "last resort" (hand-write only the up/down SQL — the two `CREATE TABLE`s above / `DROP TABLE`s — and flag in the commit that `MODELS_STATE` is stale and must be regenerated before the next model-changing migration). Prefer the real `aerich migrate`.
